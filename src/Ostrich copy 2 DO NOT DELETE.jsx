  ...
  useFrame((state, delta) => {
    ...
    if (gamepad) {
      ...
      if (Math.abs(movementX) > 0 || Math.abs(movementZ) > 0) {
        animateFromIdle();
        // Move character using your existing movement logic
        if (controller.current && body.current && collider.current) {
          try {
            const position = vec3(body.current.translation());
            const movement = vec3();

            if (movementZ > deadzone) movement.z += 0.1;
            if (movementZ < deadzone) movement.z -= 0.1;
            if (movementX < deadzone) movement.x -= 0.1;
            if (movementX > deadzone) movement.x += 0.1;
            ...
            if (movement.length() !== 0) {
              ...
              ostrichRef.current.quaternion.slerp(characterRotation, 0.05); // character ie ostrich
            }
